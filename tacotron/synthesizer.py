...
#mel_filename = synth.synthesize(text, i+1, eval_dir, log_dir, None)
    ...
    def synthesize(self, text, index, out_dir, log_dir, mel_filename):
        ...
        filename=text+'.f32'
        print(mel_filename)
        #filename=mel_filename.split('/')
        fname = filename[-1][4:-3]+'f32'
        f32name=os.path.join(log_dir,'wavs/wav-{}-me.f32'.format(index))#by jiang
        #f32name='/data/jiangpeipei/begeekmyfriend/LPCTron/Tacotron-2'+'/feaf32/'+fname
        print(f32name)#by jiang
        #npy_data.tofile(filename)#by jiang
        npy_data.tofile(f32name)#by jiang
        npy_data.tofile('/data1/jiangpeipei/begeekmyfriend/LPCTron/feature32/feature-{}.f32'.format(index))
        return
