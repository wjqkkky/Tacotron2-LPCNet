...
def yi_bian_diao(chinese_sentence_pinyin:str,word:str,split_pinyin:str,text_chinese:str,order_yi:int):
    ...
    if len(word)>1:
        ...
        for index_yi in index_list_yi:
            ...
            split_before = ' '.join(split_chinese_sentence_pinyin[all_index_list_yi[order_yi - 1]:(all_index_list_yi[order_yi - 1] + 2)])
...
def bu_bian_diao(chinese_sentence_pinyin:str,word:str,split_pinyin:str,text_chinese:str,order_bu:int):
    ...
    if len(word) > 1:
        ...
        if all_index_list_bu[order_bu-1]+1<len(text_chinese) and text_chinese[all_index_list_bu[order_bu-1]+1] !='不' :
            chinese_sentence_pinyin = chinese_sentence_pinyin.split(' ')
...
def chinese_bian_diao_pinyin(chinese:str):
    ...
    chinese_sentence_pinyin_split = PinyinHelper.convertToPinyinFromSentence(chinese, pinyinFormat=PinyinFormat.WITH_TONE_NUMBER)
    ...
    for split_chinese in split_chineses:
        # print(chinese_sentence_pinyin_split)
        # print(split_chinese)
        ...
        # print('split_pinyin',split_pinyin)
        ...
        if '一' in split_chinese or '一' == split_chinese:
            ...
            chinese_sentence_pinyin = yi_bian_diao(chinese_sentence_pinyin,split_chinese,split_pinyin,chinese,order_yi)
        if '不' in split_chinese:
            order_bu = order_bu +split_chinese.count('不')
            chinese_sentence_pinyin = bu_bian_diao(chinese_sentence_pinyin,split_chinese,split_pinyin,chinese,order_bu)
    ...
    # print(final_pinyin)
    # print(result)
    result = split_phoneme.u_to_v(final_pinyin)
    return result
