...
class PinyinResource(object):
    ...
    def getChineseResource():
        fan2jian_resource = {}
        jian2fan_resource = {}
        for line in open(os.path.join(CURRENT_DIR, 'data', 'chinese.db'),encoding='utf-8'):
            line = as_text(line.strip())
            if '=' not in line:
                continue
...
class PinyinHelper(object):
    ...
    def convertWithToneNumber(pinyin):
        ...
        for c in pinyin:
            if c in PinyinHelper.MARKED_VOWEL_TO_UNMARKED:
                converted_pinyin+=PinyinHelper.MARKED_VOWEL_TO_UNMARKED[c]
                vowel_number=PinyinHelper.MARKED_VOWEL_TO_TONE_NUMBER[c]
            else:
                converted_pinyin+=c
        if vowel_number:
            converted_pinyin+=vowel_number
        else:
            if converted_pinyin[-1].islower():
                converted_pinyin+='5'
    ...
    def convertToPinyinFromSentence(s, pinyinFormat=PinyinFormat.WITH_TONE_MARK, replace=None, segment=None):
        ...
        def __middle(s, pinyinFormat, replace):
            ...
            s = as_text(s)
            split_s = simple_seg(s)
            result_all = []
            for s in split_s:
                if RE_HANS.match(s[0]):
                    result = []
                    idx = 0
                    length = len(s)
                    while idx < length:
                        ...
                        if not isWord:
                            ...
                            if char in PinyinHelper.PINYIN_TABLE:
                                ...
                                result.append( char )
                            idx += 1
                    result_temp = [PinyinHelper.formatPinyin(pinyin, pinyinFormat) for pinyin in result]
                    result_all.extend(result_temp)
                else:
                    result_all.append(s)
